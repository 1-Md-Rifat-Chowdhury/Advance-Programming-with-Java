...
public class HomeController {
    ...
    public String showtax(Model model){

        Tax h = taxService.get(1);
        model.addAttribute("total", h.getTotal());
        model.addAttribute("taxpayble", h.getTaxpayble());
        model.addAttribute("tax", h.getTax());
        model.addAttribute("maxinvest", h.getMaxinvest());
        model.addAttribute("rebate", h.getRebate());
        model.addAttribute("nettax", h.getNettax());
        model.addAttribute("mnettax", h.getMnettax());
        return "history";
    }

    @RequestMapping("/taxcalculate")
    public String calculate(@Valid @ModelAttribute("category") String category, @ModelAttribute("zone") String zone, @ModelAttribute("salary") long salary, @ModelAttribute("hrent") long hrent, @ModelAttribute("med") long med, @ModelAttribute("con") long con, @ModelAttribute("ot") long ot, @ModelAttribute("fbonus") long fbonus, @ModelAttribute("invest") long invest, Model model) throws SQLException {
        ...
        PreparedStatement statement = connection.prepareStatement("insert into taxcalculator(total,taxpayble,tax,maxinvest,rebate,nettax,mnettax) values (?,?,?,?,?,?,?)");
        statement.setLong(1, total);
        statement.setLong(2, taxpayable);
        statement.setLong(3, tax);
        statement.setLong(4, maxinvest);
        statement.setLong(5, rebate);
        statement.setLong(6, nettax);
        statement.setLong(7, mnettax);
        statement.execute();
